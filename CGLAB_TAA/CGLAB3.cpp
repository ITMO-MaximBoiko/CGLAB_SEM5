...
void CGLAB::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for (auto& rItem : mAllRitems)
	{
		if (rItem->name == "skull")
		{
			XMMATRIX scale = XMMatrixScaling(4., 4., 4.);
			XMMATRIX rotate = XMMatrixRotationY(gt.TotalTime());
			XMMATRIX translate = XMMatrixTranslation(0., 4., 0.);
...
void CGLAB::BuildLights()
{
	...
	// other
	CreatePointLight({ 0,3,-2 }, { 4,0,0 }, 1, 5, 1);
